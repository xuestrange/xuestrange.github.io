...
The compact timeline above the table shows past and upcoming meetings in date order, with clickable entries that jump to their table rows. Upcoming and ongoing rows are bold and past rows are dimmed but remain legible. On the Links page, `assets/js/conference-dates.js` uses the viewer's local calendar date for the Today marker, status labels, and row ordering. It refreshes when opened, at local midnight, and when the page becomes active again, without requiring a site rebuild. `_includes/conference-status.html` supplies a build-date fallback labeled “As of” when JavaScript is unavailable. `checked_on` records the separate date when official sources were last verified and is not changed by the script. Meetings remain ongoing through their end date, inclusive. Use `null` for both dates when an edition has been announced without exact dates; editions missing either date are shown separately as pending, without inventing a position on the timeline. Conference details still require source verification; the website does not automatically fetch new announcements or send notifications.
